...
Insights
Settings
2022.2-GetPet/getpet_app/lib/Widgets/FazerChamado.dart
@murilopbs
murilopbs Add files via upload
 1 contributor
496 lines (490 sloc)  19.8 KB
          ...
          return LayoutBuilder(
            builder: (context, constrains) => ListView(
              ...
              children: [
                ...
                const Center(
                    child:  Text("Realizar Chamado:", style: TextStyle(fontSize: 28),)
                ),
                const SizedBox(height: 15,),
                    ...
                    onPressed: () async{
                      if(longitude == 666){
                        ...
                        DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection(
                            'usuarios').doc(model.firebaseUser!.uid).get();
                        final DateTime now = DateTime.now();
                        final DateFormat formatter = DateFormat('dd/MM/yyyy');
                        final String formatted = formatter.format(now);
                        await FirebaseFirestore.instance.collection("usuarios").doc(model.firebaseUser!.uid).collection("pedidos").doc().set(
                            {
